...
class Snap extends CI_Controller
{
	...
	{
		$result = json_decode($this->input->post('result_data'), true);
		$checked =  htmlspecialchars($this->input->post('checkhadiah', true));
		$order_id = $checked === 'true' ? $result['order_id'] : 0;

		$kode_undangan = htmlspecialchars($this->input->post('kode_undangan', true));
		$url_undangan = htmlspecialchars($this->input->post('url_undangan', true));
		...
		if (count($data_email) > 0 || count($data_wa) > 0) {
			...
			$this->emailmatch($ucapan, $id_tamu, $order_id);
		} else {
			//Kondisi jika wa dan email tidak ada yang sama
			$this->emailnotmatch($kode_undangan, $nama, $email, $wa, $ucapan, $order_id);
		}
		...
		// if ($simpan) {
		redirect('Undangan/' . $url_undangan);
		// redirect('A');
		// } else {
		// 	echo "Gagal Simpan 1";
		// }
	...
	public function emailnotmatch($kode_undangan, $nama, $email, $wa, $ucapan, $id_pembayaran)
	{
		/* 
		Function ini berfungsi untuk insert ke tb_tamu dan tb_ucapan 
		function ini berfungsi kalau wa dan email tidak di temukan di dalam tb_tamu 
		*/
		$data_tamu = [
		...
		$this->Ucapan_Model->insert_to_tb_ucapan($data);
	}

	public function emailmatch($ucapan, $Id_Tamu, $Id_Pembayaran)
	{
		$data = [
			'Ucapan' => $ucapan,
			'Id_Tamu' => $Id_Tamu,
